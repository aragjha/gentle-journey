...
const CTAButton = forwardRef<HTMLButtonElement, CTAButtonProps>(({
  ...
  return <motion.button ref={ref} type={type} whileTap={{
    scale: 0.98,
    y: 2
  }} transition={{
    duration: 0.1
  }} className={cn(baseStyles, variants[variant], sizes[size], disabled && "opacity-40 cursor-not-allowed", className)} disabled={disabled || isLoading} onClick={onClick}>CONTINUE{isLoading ? <motion.div className="w-6 h-6 border-2 border-current border-t-transparent rounded-full" animate={{
      rotate: 360
    }} transition={{
      duration: 1,
      repeat: Infinity,
